module Starcharter
  module Store
    module Base
      ...
      #
      def to_coordinates
        [:x, :y].map{ |i| send self.class.starcharter_options[i] }
      end
      ...
      def distance_to(point, units = nil)
        units ||= self.class.starcharter_options[:units]
        return nil unless starcharted?
        Starcharter::Calculations.distance_between(
      ...
      def bearing_to(point, options = {})
        options[:method] ||= self.class.starcharter_options[:method]
        return nil unless starcharted?
        Starcharter::Calculations.bearing_between(
